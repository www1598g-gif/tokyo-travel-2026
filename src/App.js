            ...
            {isMember && (
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-stone-500">{UTILS_DATA.accommodation.keyNote}，退房記得把鑰匙放回鎖好。</p>
                </div>
              </div>
            )}
          </div>
...
const GuidePage = ({ isAdmin, isMember, noticeText, updateNoticeText }) => {
  ...
  const startEditRes = (i) => { setEditingRes(i); setEditDraft({ ...reservations[i] }); };
  const cancelEditRes = () => { setEditingRes(null); setEditDraft(null); };
  const saveEditRes = () => {
    const newList = [...reservations];
    newList[editingRes] = editDraft;
  ...
  const moveRes = (i, dir) => {
    ...
    if (newIndex < 0 || newIndex >= reservations.length) return;
    const newList = [...reservations];
    [newList[i], newList[newIndex]] = [newList[newIndex], newList[i]];
    saveReservations(newList);
  };
  const deleteRes = (i) => {
    if (!window.confirm('確定刪除這筆預約？')) return;
    saveReservations(reservations.filter((_, idx) => idx !== i));
  };
...
<a href="https://www.coinlocker-navi.com/" target="_blank" rel="noreferrer" className="flex items-center justify-between p-3 bg-stone-50 rounded-xl border border-stone-100 active:scale-95 transition-all">
  <div><div className="font-bold text-stone-800 text-sm">コインロッカーなび</div><div className="text-[10px] text-stone-500">查詢車站投幣式置物櫃即時空位</div></div>
  <ArrowRight size={16} className="text-stone-400" />
</a>
